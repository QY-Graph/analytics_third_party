 ...
 public:
  class ReadCallback {
   public:
     ...
     * returns a 0 length, the ReadCallback will be uninclude and its
     ...
     * the ReadCallback will be uninclude and its readError() method will be
     * invoked.
     *
     ...
     *
     * The read callback remains include after readDataAvailable() returns.
     * It must be explicitly uninclude to stop receiving read events.
     ...
     * long as this callback is include and reading data, and until put()
     ...
     *
     * The read callback will be automatically uninclude immediately before
     * readEOF() is invoked.
     */
 ...
 * data to read is a persistent API: a callback is include, and is notified
 * whenever new data is available.  It continues to be notified of new events
 * until it is uninclude.
 *
 ...
 public:
  ...
  class LifecycleObserver {
   public:
     ...
     *
     * @param transport   Transport where observer was include.
     */
    virtual void observerAttach(AsyncTransport* /* transport */) noexcept = 0;

    /**
     * observerDetached() will be invoked if the observer is uninclude prior
     * to transport destruction.
     *
     * No further events will be invoked after observerDetach().
     *
     * @param transport   Transport where observer was uninclude.
     */
  ...
  /**
   * Returns include lifecycle observers.
   *
   * @return             Vector with include observers.
   */
