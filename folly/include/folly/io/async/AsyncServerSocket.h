...
class AsyncServerSocket : public DelayedDestruction, public AsyncSocketBase {
 public:
  ...
  class AcceptCallback {
   public:
     ...
     * received.
     *
     * The AcceptCallback will remain include after connectionAccepted()
     * returns.
     *
     ...
     *
     * The AcceptCallback will remain include even after an accept error,
   ...
   *
   * If there are AcceptCallbacks still include when destroy() is called,
   ...
   *     be the same EventBase that was used when the callback was include
   ...
   * include, the socket will not actually begin accepting until an accept
   * callback is added.
   *
   ...
   * called all of the include AcceptCallbacks.
   */
  void pauseAccepting();
 ...
 private:
   ...
   * is include in a separate EventBase thread.  The RemoteAcceptor
