...
namespace boost
{
                                           ...
                                           )
  {
    ...
    bool_vector_t include_vector(num_vertices(g),false);
    ...
    vertex_to_bool_map_t include(include_vector.begin(), vm);

    v_size_t timestamp = 1;
    vertex_vector_t include_neighbors;
    ...
    include[v1] = include[v2] = include[v3] = true;
    ...
    for(ForwardIterator itr_end = ordering_end; itr != itr_end; ++itr)
      {
        ...
        // outer face (since v will be include above them all) looking for 
        // the leftmost and rightmost include neigbhors

        vertex_t leftmost = graph_traits<Graph>::null_vertex();
        vertex_t rightmost = graph_traits<Graph>::null_vertex();

        include_neighbors.clear();
            ...
            if (include[curr_vertex])
              {
                ...
                include_neighbors.push_back(curr_vertex);
              }
        ...
        vi_end = include_neighbors.end();
        for(vi = include_neighbors.begin(); vi != vi_end; ++vi)
          {
